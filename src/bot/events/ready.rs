...
pub async fn ready(ctx: Context, ready: serenity::model::gateway::Ready) {
    ...
    for guild in ctx.cache.guilds() {
        if !db
            .allowed_guild_exists(guild.0.to_string().as_str())
            .await
        ...
        let _commands = GuildId::set_application_commands(&guild, &ctx.http, |commands| {
            commands
                ...
                .create_application_command(|command| {
                    commands::remove_auto_emoji::register(command)
                })
                ...
                .create_application_command(|command| commands::show_channels::register(command))
